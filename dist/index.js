...
// FIX 2: Never fall back to wildcard CORS — explicit allowlist only
...
async function auditLog(userId, action, targetId, details = '', req) {
    try {
...
app.post('/api/auth/logout', authenticate, asyncHandler(async (req, res) => {
    await auditLog(req.user.id, 'AUTH_LOGOUT', req.user.id, `Logged out (roles: ${req.user.roles.join(', ')})`, req);
...
app.post('/api/auth/forgot-password', asyncHandler(async (req, res) => {
    ...
    if (!user)
        return res.json({ success: true, message: 'If that email exists, a reset request has been created.' });
    // Cancel any existing unused reset requests for this user
    await exports.prisma.passwordResetRequest.updateMany({
    ...
    await exports.prisma.passwordResetRequest.create({
...
app.get('/api/admin/password-resets', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), asyncHandler(async (req, res) => {
    const requests = await exports.prisma.passwordResetRequest.findMany({
...
app.delete('/api/admin/password-resets/:id', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), asyncHandler(async (req, res) => {
    await exports.prisma.passwordResetRequest.update({
...
app.post('/api/auth/reset-password', asyncHandler(async (req, res) => {
    ...
    const request = await exports.prisma.passwordResetRequest.findFirst({
        where: {
            userId: user.id,
            resetCode,
            isUsed: false,
    ...
    const currentUser = await exports.prisma.user.findUnique({ where: { id: user.id }, select: { passwordHash: true } });
    if (currentUser)
        await savePasswordHistory(user.id, currentUser.passwordHash);
    // Mark request as used, update password
    await exports.prisma.passwordResetRequest.update({ where: { id: request.id }, data: { isUsed: true } });
    const passwordHash = await bcrypt_1.default.hash(newPassword, 12);
    await exports.prisma.user.update({ where: { id: user.id }, data: { passwordHash } });
...
async function checkPasswordHistory(userId, newPassword) {
    // Get current hash
    const user = await exports.prisma.user.findUnique({ where: { id: userId }, select: { passwordHash: true } });
    if (user && await bcrypt_1.default.compare(newPassword, user.passwordHash)) {
        return { reused: true, message: 'New password cannot be the same as your current password.' };
    }
    // Get last 3 historical hashes
    const history = await exports.prisma.passwordHistory.findMany({
        where: { userId },
        orderBy: { createdAt: 'desc' },
        take: 3,
    ...
    for (const h of history) {
        if (await bcrypt_1.default.compare(newPassword, h.passwordHash)) {
...
async function savePasswordHistory(userId, currentHash) {
    await exports.prisma.passwordHistory.create({ data: { userId, passwordHash: currentHash } });
    // Keep only last 3 — delete older ones
    const all = await exports.prisma.passwordHistory.findMany({
        where: { userId }, orderBy: { createdAt: 'desc' }, select: { id: true }
    });
    if (all.length > 3) {
        const toDelete = all.slice(3).map(h => h.id);
        await exports.prisma.passwordHistory.deleteMany({ where: { id: { in: toDelete } } });
    }
}
// ---------- AUTH ----------
app.post('/api/auth/login', loginLimiter, async (req, res) => {
    ...
    const valid = await bcrypt_1.default.compare(password, hashToCompare);
    if (!user || !valid || !user.isActive)
        return res.status(401).json({ error: 'Invalid credentials' });
    const token = jsonwebtoken_1.default.sign({ userId: user.id }, process.env.JWT_SECRET, { expiresIn: '8h' });
    // Check if user has a pending CLI recovery reset — flag them to change password
    const pendingRecovery = await exports.prisma.passwordResetRequest.findFirst({
    ...
    if (pendingRecovery) {
        await exports.prisma.passwordResetRequest.update({ where: { id: pendingRecovery.id }, data: { isUsed: true } });
    }
    // AUTH log — every login recorded
    await auditLog(user.id, 'AUTH_LOGIN', user.id, `Logged in as ${user.userRoles.map((ur) => ur.role.name).join(', ')}`, req);
    res.json({
        token,
...
// ---------- STUDENT EXAM ROUTES ----------
app.get('/api/exams/available', authenticate, requireRole(['Student']), async (req, res) => {
    const now = new Date();
    ...
    const exams = await exports.prisma.exam.findMany({
        where: {
            isPublished: true,
            startTime: { lte: now },
            endTime: { gte: now }
...
app.post('/api/exams/:examId/start', authenticate, requireRole(['Student']), async (req, res) => {
    const { examId } = req.params;
    const studentId = req.user.id;
    // Count submitted attempts for this student & exam
    const submittedCount = await exports.prisma.examAttempt.count({
    ...
    const studentRecord = await exports.prisma.user.findUnique({ where: { id: studentId } });
    if (!studentRecord?.isActive) {
        return res.status(403).json({ error: 'Your account has been deactivated. Contact your administrator.' });
    }
    const existingAttempt = await exports.prisma.examAttempt.findFirst({
        where: { studentId, examId, status: { in: ['IN_PROGRESS', 'PAUSED'] } }
    });
    if (existingAttempt) {
        const exam = await exports.prisma.exam.findUnique({
            where: { id: examId },
        ...
        try {
            ...
            if (saved.questionOrder && saved.questionOrder.length > 0) {
                const orderMap = {};
                saved.questionOrder.forEach((id, idx) => { orderMap[id] = idx; });
    ...
    const attempt = await exports.prisma.examAttempt.create({
    ...
    await exports.prisma.answer.createMany({
    ...
    await exports.prisma.examAttempt.update({
        where: { id: attempt.id },
...
app.get('/api/my-results', authenticate, requireRole(['Student']), async (req, res) => {
    const studentId = req.user.id;
    const results = await exports.prisma.result.findMany({
...
app.get('/api/results/:attemptId/review', authenticate, async (req, res) => {
    const { attemptId } = req.params;
    const studentId = req.user.id;
    const attempt = await exports.prisma.examAttempt.findFirst({
...
app.patch('/api/attempts/:attemptId/auto-save', authenticate, async (req, res) => {
    const { attemptId } = req.params;
    const { answers, questionOrder } = req.body;
    const attempt = await exports.prisma.examAttempt.findUnique({ where: { id: attemptId } });
    if (!attempt)
...
app.post('/api/attempts/:attemptId/submit', authenticate, async (req, res) => {
    ...
    const attempt = await exports.prisma.examAttempt.findUnique({
        where: { id: attemptId },
        include: {
            exam: true,
            answers: {
    ...
    for (const ans of attempt.answers) {
        ...
        const updateData = {};
        if (q.type === 'MCQ' || q.type === 'TRUE_FALSE') {
            const selectedOpt = q.options.find((opt) => opt.id === studentAnswer);
        ...
        await exports.prisma.answer.update({ where: { id: ans.id }, data: updateData });
    }
    ...
    const result = await exports.prisma.result.create({
        data: {
            attemptId,
            totalPoints,
            obtainedPoints,
...
app.get('/api/results/:attemptId', authenticate, async (req, res) => {
    const result = await exports.prisma.result.findUnique({
...
app.patch('/api/admin/exams/:examId/publish', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    const { examId } = req.params;
    const { isPublished } = req.body;
    const exam = await exports.prisma.exam.update({
        where: { id: examId },
...
app.patch('/api/admin/exams/:examId/extend', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    const { examId } = req.params;
    ...
    const existing = await exports.prisma.exam.findUnique({ where: { id: examId } });
    if (!existing)
        return res.status(404).json({ error: 'Exam not found' });
...
app.get('/api/admin/exams', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin', 'Lecturer']), async (req, res) => {
    const exams = await exports.prisma.exam.findMany({ orderBy: { createdAt: 'desc' }, include: { questions: true } });
    res.json(exams);
});
app.post('/api/admin/exams', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    ...
    const exam = await exports.prisma.exam.create({
        data: {
...
app.post('/api/admin/exams/:examId/questions', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin', 'Lecturer']), async (req, res) => {
    const { examId } = req.params;
    ...
    const question = await exports.prisma.question.create({
        data: {
            examId, text: sanitizeQuestionHtml(text), type, points, orderIndex,
            options: { create: options.map((opt) => ({ text: opt.text, isCorrect: opt.isCorrect, orderIndex: opt.orderIndex })) }
        }
    });
...
app.post('/api/admin/exams/:examId/import-questions', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin', 'Lecturer']), async (req, res) => {
    const { examId } = req.params;
    ...
    const exam = await exports.prisma.exam.findUnique({ where: { id: examId }, include: { questions: true } });
    if (!exam)
        return res.status(404).json({ error: 'Exam not found' });
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (!['MCQ', 'TRUE_FALSE', 'FILL_BLANK', 'THEORY'].includes(type)) {
            errors.push(`Row ${rowNum}: invalid type "${row.type}". Must be MCQ, TRUE_FALSE, FILL_BLANK, or THEORY`);
            continue;
        }
        let options = [];
        if (type === 'MCQ') {
            ...
            if (opts.length < 2) {
                errors.push(`Row ${rowNum}: MCQ needs at least 2 options (option_a, option_b...)`);
                continue;
            }
            const correct = (row.correct || '').toString().trim().toUpperCase();
            const correctIdx = ['A', 'B', 'C', 'D'].indexOf(correct);
            if (correctIdx === -1) {
                errors.push(`Row ${rowNum}: correct must be A, B, C, or D for MCQ (got "${row.correct}")`);
                continue;
            }
            ...
            if (!['TRUE', 'FALSE'].includes(correct)) {
                errors.push(`Row ${rowNum}: correct must be TRUE or FALSE for TRUE_FALSE`);
                continue;
            }
            options = [
            ...
            if (!answer) {
                errors.push(`Row ${rowNum}: FILL_BLANK needs the correct answer in the "correct" column`);
                continue;
            }
...
app.get('/api/admin/exams/:examId/questions', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin', 'Lecturer']), async (req, res) => {
    const questions = await exports.prisma.question.findMany({ where: { examId: req.params.examId }, include: { options: true }, orderBy: { orderIndex: 'asc' } });
    res.json(questions);
});
app.put('/api/admin/questions/:questionId', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin', 'Lecturer']), async (req, res) => {
    const { questionId } = req.params;
    const { text, type, points, orderIndex, options } = req.body;
    await exports.prisma.option.deleteMany({ where: { questionId } });
    const updated = await exports.prisma.question.update({
        where: { id: questionId },
        data: { text, type, points, orderIndex, options: { create: options.map((opt) => ({ text: opt.text, isCorrect: opt.isCorrect, orderIndex: opt.orderIndex })) } }
    });
    res.json(updated);
});
app.delete('/api/admin/questions/:questionId', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    const { questionId } = req.params;
    await exports.prisma.option.deleteMany({ where: { questionId } });
    await exports.prisma.answer.deleteMany({ where: { questionId } });
    await exports.prisma.question.delete({ where: { id: questionId } });
    await auditLog(req.user.id, 'QUESTION_DELETE', questionId, 'Question deleted', req);
    res.json({ success: true });
});
app.post('/api/admin/users', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
...
app.get('/api/admin/users', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
...
// GET single user
app.get('/api/admin/users/:userId', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    const user = await exports.prisma.user.findUnique({
...
app.put('/api/admin/users/:userId', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    ...
    try {
        ...
        if (password && password.length >= 6) {
            ...
            if (userId === req.user.id) {
                ...
                const adminUser = await exports.prisma.user.findUnique({ where: { id: userId } });
                const valid = adminUser ? await bcrypt_1.default.compare(currentPassword, adminUser.passwordHash) : false;
                if (!valid)
            ...
            // Save current password to history before overwriting
            const targetUser = await exports.prisma.user.findUnique({ where: { id: userId }, select: { passwordHash: true } });
            if (targetUser)
                await savePasswordHistory(userId, targetUser.passwordHash);
            updateData.passwordHash = await bcrypt_1.default.hash(password, 12);
        }
        const user = await exports.prisma.user.update({ where: { id: userId }, data: updateData });
        // Update role if provided — use explicit roleId to satisfy Prisma's strict types
        if (roleName) {
            const role = await exports.prisma.role.upsert({
                where: { name: roleName },
                update: {},
                create: { name: roleName }
            });
            await exports.prisma.userRole.deleteMany({ where: { userId } });
            await exports.prisma.userRole.create({ data: { userId, roleId: role.id } });
        }
...
app.delete('/api/admin/users/:userId', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    const { userId } = req.params;
    if (userId === req.user.id) {
        return res.status(400).json({ error: 'You cannot delete your own account' });
    }
    // Delete in dependency order
    await exports.prisma.answer.deleteMany({ where: { attempt: { studentId: userId } } });
    await exports.prisma.result.deleteMany({ where: { attempt: { studentId: userId } } });
    await exports.prisma.examAttempt.deleteMany({ where: { studentId: userId } });
    await exports.prisma.userRole.deleteMany({ where: { userId } });
    await exports.prisma.auditLog.deleteMany({ where: { userId } });
    await exports.prisma.user.delete({ where: { id: userId } });
    await auditLog(req.user.id, 'DELETE_USER', userId);
    res.json({ success: true });
});
app.put('/api/admin/exams/:examId', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    const { examId } = req.params;
    const { title, description, startTime, endTime, durationMinutes, shuffleQuestions, shuffleOptions, negativeMarkingPercent, passingScorePercent } = req.body;
    const start = new Date(startTime);
    const end = new Date(endTime);
    if (isNaN(start.getTime()) || isNaN(end.getTime()))
        return res.status(400).json({ error: 'Invalid date' });
    const updated = await exports.prisma.exam.update({
        where: { id: examId },
...
app.delete('/api/admin/exams/:examId', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    const { examId } = req.params;
    const exam = await exports.prisma.exam.findUnique({ where: { id: examId } });
    if (!exam)
        return res.status(404).json({ error: 'Exam not found' });
    // Batch deletes in dependency order — replaces old N+1 per-question/per-attempt loops
    await exports.prisma.answer.deleteMany({ where: { attempt: { examId } } });
    await exports.prisma.result.deleteMany({ where: { attempt: { examId } } });
    await exports.prisma.examAttempt.deleteMany({ where: { examId } });
    await exports.prisma.option.deleteMany({ where: { question: { examId } } });
    await exports.prisma.question.deleteMany({ where: { examId } });
    await exports.prisma.exam.delete({ where: { id: examId } });
    await auditLog(req.user.id, 'DELETE_EXAM', examId);
    res.json({ success: true });
});
// Admin: Reset (delete) a student's attempt so they can retake the exam
app.delete('/api/admin/attempts/:attemptId', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), async (req, res) => {
    const { attemptId } = req.params;
    try {
        // Delete associated answers, result, then the attempt itself
        await exports.prisma.answer.deleteMany({ where: { attemptId } });
        await exports.prisma.result.deleteMany({ where: { attemptId } });
        await exports.prisma.examAttempt.delete({ where: { id: attemptId } });
...
app.get('/api/admin/attempts', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin', 'Lecturer']), async (req, res) => {
    const attempts = await exports.prisma.examAttempt.findMany({
...
app.get('/api/admin/reports/overview', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), asyncHandler(async (req, res) => {
    const [totalExams, totalStudents, totalAttempts, passedAttempts] = await Promise.all([
        exports.prisma.exam.count(),
        exports.prisma.user.count({ where: { userRoles: { some: { role: { name: 'Student' } } } } }),
        exports.prisma.examAttempt.count({ where: { status: 'SUBMITTED' } }),
        exports.prisma.result.count({ where: { isPassed: true } }),
    ]);
...
app.get('/api/admin/reports/exams', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), asyncHandler(async (req, res) => {
    const exams = await exports.prisma.exam.findMany({
        select: {
    ...
    const report = exams.map(exam => {
        ...
        const results = attempts.map(a => a.result).filter(Boolean);
...
app.get('/api/admin/reports/students', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), asyncHandler(async (req, res) => {
    const students = await exports.prisma.user.findMany({
...
app.get('/api/admin/reports/exams/:examId/questions', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), asyncHandler(async (req, res) => {
    const { examId } = req.params;
    const questions = await exports.prisma.question.findMany({
        where: { examId },
        include: {
            options: true,
            answers: {
...
// ═══════════════════════════════════════════════════════════════════════════
app.get('/api/admin/audit-logs', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), asyncHandler(async (req, res) => {
    const { category, userId, from, to, page = '1', limit = '50' } = req.query;
    ...
    const [logs, total] = await Promise.all([
        exports.prisma.auditLog.findMany({
            where,
            include: {
        ...
        exports.prisma.auditLog.count({ where })
    ]);
    res.json({ logs, total, page: pageNum, totalPages: Math.ceil(total / limitNum) });
}));
// CSV export of audit logs — admin only
app.get('/api/admin/audit-logs/export', authenticate, requireRole(['SuperAdmin', 'SchoolAdmin']), asyncHandler(async (req, res) => {
